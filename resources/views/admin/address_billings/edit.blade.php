<div class="modalhide">
    <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">{{ trans('app.edit_address') }}
            : {{ $address->friendly_name }}<br><small>{{ $profile->business_name }}</small></h4>
    </div>
    <form role="form" method="POST" action="{{ url(Session::get('guard') . '/profiles/'.$profile->id.'/address_billings') }}/{{ $address->id }}"
          id="idForm" reloadiv="{{ Request::segment(7) }}"
          reloadurl="{{ url(Session::get('guard') . '/profiles/'.$profile->id.'/address_billings') }}">
