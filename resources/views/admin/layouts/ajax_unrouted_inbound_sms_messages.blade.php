            ...
            <ul class="dropdown-menu mailbox">
                <li>
                    <div class="drop-title">
                        {{ count($inbound_messages) }} {{ trans_choice('app.choice_unrouted_new_sms_message',count($inbound_messages)) }}
                        <br>
                ...
                <li>
                    <div class="message-center">
                        @foreach($inbound_messages as $oneMessage)
                            <a href="{{ url(Session::get('guard') . '/ajaxdata/unrouted_inbound_sms_messages/' . $oneMessage['id']. '/read') }}" class="fetchajaxpage" data-reloaddiv="ajax_unrouted_inbound_sms_messages">
                                <div class="mail-contnet">
                                    <h5>{{ trans('app.from') }} +{{ $oneMessage['from_number'] }}
                                        <small>{{ trans('app.via') }} {{ $oneMessage['gateway'] }}</small>
                                    </h5>
                                    <span style="font-size:12px;display:block;margin:5px 0;color:#2b2b2b">{{ $oneMessage['message'] }}</span>
