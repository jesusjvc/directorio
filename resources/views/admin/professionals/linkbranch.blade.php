<div class="modalhide">
   ...
   @else
    <form role="form" method="POST" action="{{ url(Session::get('guard') . '/' . $profile->id . '/professionals/' . $professional->id . '/linkbranch') }}" id="idForm"
          reloadiv="reload" reloadurl="{{ url(Session::get('guard') . '/' . $profile->id . '/professionals') }}">
        <div class="modal-body">
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
