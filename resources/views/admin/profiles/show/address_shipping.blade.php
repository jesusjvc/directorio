        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('app.profile') }} {{ trans_choice('app.shipping_address',count($profile->address_shippings)) }}
                    <div class="pull-right">
                        <span class="btn btn-xs btn-success" data-toggle="modal" data-target="#ajaxmodel"
                              data-remote="{{ url(Session::get('guard') . '/profiles/' . $profile->id . '/address_shippings/create/' . $sectiontoreload) }}">
                ...
                <div class="panel-body">
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                            @foreach($profile->address_shippings as $address)
                                <tr>
                                    <td>
                                    ...
                                    <td>
                                        <span class="btn btn-primary btn-xs" data-toggle="modal"
                                              ...
                                              data-remote="{{ url(Session::get('guard') . '/profiles/' . $profile->id . '/address_shippings/' . $address->id . '/edit/' . $sectiontoreload) }}">{{ trans('app.edit') }}</span>
                                    </td>
                                    <td>
                                        @if($address->is_default != 1)
                                            <span class="btn btn-danger btn-xs postconfirm"
                                                  ...
                                                  data-reloadurl="{{ url(Session::get('guard') . '/profiles/' . $profile->id . '/address_shippings') }}"
                                                  data-posturl="{{ url(Session::get('guard') . '/profiles/' . $profile->id . '/address_shippings/' . $address->id) }}">{{ trans('app.delete') }}</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if($address->is_default != 1)
                                            <span class="btn btn-danger btn-xs confirmajaxpost"
                                                  ...
                                                  data-description="{{ trans('app.are_you_sure_you_want_to_set_the_address_theaddress_as_the_default_shipping_address',['theaddress' => "<i>" . $address->friendly_name . "</i>"]) }}"
                                                  data-posturl="{{ url(Session::get('guard') . '/profiles/' . $profile->id . '/address_shippings/' . $address->id . '/setasdefault') }}"
                                                  data-reloadurl="{{ url(Session::get('guard') . '/profiles/' . $profile->id . '/address_shippings') }}">{{ trans('app.set_as_default') }}</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
