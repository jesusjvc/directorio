...
<div class="modalhide">
    ...
    <form role="form" method="POST"
          action="{{ url(Session::get('guard') . '/' . $profile->id . '/' . $section . '/' . $user->id) }}" id="idForm"
          reloadiv="{{ Request::segment(6) }}"
          reloadurl="{{ url(Session::get('guard') . '/' . $profile->id . '/' . $section) }}">
        <div class="modal-body">
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
            @if(($user->isprofessional == 1) && ($user->isuser == 1) && ($section == 'users'))
                <p class="text-muted">
                    {{ trans('app.usernames_is_assigned_as_a_professional_as_well_if_you_change_firstnames_credentials_here_the_same_changes_will_apply_to_firstnames_professional_section', ["usernames" => ucwords(trans('app.' . $user->prefix) . ' ' . $user->firstname . ' ' . $user->lastname), "firstname" => ucwords($user->firstname)]) }}
                </p>
                <hr>
            @endif
            @if(($user->isprofessional == 1) && ($user->isuser == 1) && ($section == 'professionals'))
                <p class="text-muted">
                    {{ trans('app.usernames_is_assigned_as_a_user_as_well_if_you_change_firstnames_credentials_here_the_same_changes_will_apply_to_firstnames_user_section', ["usernames" => ucwords(trans('app.' . $user->prefix) . ' ' . $user->firstname . ' ' . $user->lastname), "firstname" => ucwords($user->firstname)]) }}
                </p>
                <hr>
            @endif
            <div class="row">
