<div class="modalhide">
    <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">{{ trans('app.edit_billing_address') }} : {{ $address->friendly_name }}
            <br>
            <small>{{ ucwords($address->customer->prefix . ' ' . $address->customer->firstname . ' ' . $address->customer->lastname) }}</small></h4>
    </div>
    <form role="form" method="POST" action="{{ url(Session::get('guard') . '/customer_billing_address/' . $address->id) }}" id="idForm"
