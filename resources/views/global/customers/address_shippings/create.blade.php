<div class="modalhide">
    <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">{{ trans('app.register_a_new_shipping_address') }}
            <br>
        <small>{{ ucwords(trans('app.' . $customer->prefix) . ' ' . $customer->firstname . ' ' . $customer->lastname) }}</small></h4>
    </div>
    <form role="form" method="POST" action="{{ url(Session::get('guard') . '/customer_shipping_address/' . $customer->id) }}" id="idForm"
          @if(Session::get('guard') == 'customer')
          reloadurl="{{ url(Session::get('guard') . '/address_management') }}"
          @else
          reloadurl="{{ url(Session::get('guard') . '/customers/' . $customer->id) }}"
            @endif
    >
