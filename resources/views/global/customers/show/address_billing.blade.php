    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                {{ trans('app.customer_names_billing_addresses',["customer_names" => ucwords(trans('app.' . $customer->prefix) . ' ' . $customer->firstname . ' ' . $customer->lastname)]) }}
                <div class="pull-right">
                        <span class="btn btn-xs btn-success" data-toggle="modal" data-target="#ajaxmodel"
                              data-remote="{{ url(Session::get('guard') . '/customer_billing_address/' . $customer->id . '/create') }}">
            ...
            <div class="panel-body">
                <div class="table-responsive">
                    @if(count($customer->address_billings) > 0)
                        <table class="table">
                            <thead>
                            <tr>
                                <th>
                            ...
                            <tbody>
                            @foreach($customer->address_billings as $address)
                                <tr>
                                    <td>
                                    ...
                                    <td>
                                        <span class="btn btn-primary btn-xs" data-toggle="modal"
                                              ...
                                              data-remote="{{ url(Session::get('guard') . '/customer_billing_address/' . $address->id.'/edit') }}">{{ trans('app.edit') }}
                                        </span>
                                    </td>
                                    <td>
                                        @if($address->is_default != 1)
                                            <span class="btn btn-danger btn-xs postconfirm"
                                                  ...
                                                  @endif
                                                  data-posturl="{{ url(Session::get('guard') . '/customer_billing_address/'.$address->id) }}">{{ trans('app.delete') }}</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if($address->is_default != 1)
                                            <span class="btn btn-danger btn-xs confirmajaxpost"
                                                  ...
                                                  data-posturl="{{ url(Session::get('guard') . '/customer_billing_address/'.$address->id.'/setasdefault') }}"
                                                  @if(Session::get('guard') == 'customer')
                                                  data-reloadurl="{{ url(Session::get('guard') . '/address_management') }}"
                                                  @else
                                                  data-reloadurl="{{ url(Session::get('guard') . '/customers/' . $customer->id) }}"
