<div class="modal-header">
    ...
    <h4 class="modal-title" id="myModalLabel">{{ trans('app.edit_payment_gateway') }}
        : {{ $gateway->payment_method_name }} @if(isset($profile))<br><small> {{ $profile->business_name }} </small> @endif</h4>
</div>
@php
    if(isset($profile)):
...
@endphp
<form role="form" method="POST" action="{{ url(Session::get('guard') . '/' . $modifylink . 'payment_gateways/'.$gateway->id) }}" id="idForm"
      reloadurl="{{ url(Session::get('guard') . '/' . $modifylink . 'payment_gateways') }}">
        ...
        <div class="form-body">
            <div class="form-group">
                ...
                <select class="form-control" name="static_payment_gateway_id" id="static_payment_gateway_id" required>
                    ...
                    @foreach ($static_gateways as $instance)
                        <option value="{{ $instance->id }}"
                                @if($instance->id == $gateway->static_payment_gateway_id) selected
                                @endif populate="{{ $instance->required_variables }}"
            ...
            <div style="display:none" id="additionalinfodiv">
                <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" maxlength="100" name="payment_method_name"
                       value="{{ $gateway->payment_method_name }}" class="form-control" required>
            </div>
            <div class="form-group">
                <label>{{ trans('app.variable_values') }} </label>
                <input type="text" id="variable_values" maxlength="1500" name="variable_values"
                       value="{{ $gateway->variable_values }}"
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" rows="5" name="instructions"
                          placeholder="{{ trans('app.this_is_ideal_for_cases_where_you_need_to_provide_additional_payment_instructions_such_as_bank_account_details_for_bank_payments_leave_blank_if_you_do_not_want_to_provide_any_payment_instructions') }}">{{ $gateway->instructions }}</textarea>
            </div>
            {{--<div class="form-group">--}}
                {{--<label>--}}
                    {{--<input name="include_instructions_in_notification" value="1"--}}
                           {{--@if($gateway->include_instructions_in_notification == 1) checked--}}
                           {{--@endif type="checkbox"> {{ trans('app.include_payment_instructions_in_the_notification_emailsms') }}--}}
                {{--</label>--}}
            {{--</div>--}}
        </div>
    </div>
