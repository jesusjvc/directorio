            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    ...
                    <div class="pull-right">
                        <a href="{{ url(Session::get('guard') . '/branches/' . $branch->id) }}"
                           class="fetchajaxpage">
                        ...
                        <span class="btn btn-success btn-xs" data-toggle="modal" data-target="#ajaxmodel"
                              data-remote="{{ url(Session::get('guard') . '/did_number/' . $branch->id . '/purchase') }}">{{ trans('app.buy_a_number') }}
                                </span>
                    </div>
                </div>
                    ...
                    @else
                        @foreach($branch->app_sms_did_numbers as $number)
                            ...
                            <span class="btn btn-danger btn-xs postconfirm"
                                      ...
                                      data-reloadurl="{{ url(Session::get('guard') . '/did_number/' . $branch->id) }}"
                                      data-posturl="{{ url(Session::get('guard') . '/did_number/' . $branch->id . '/release/' . $number->id) }}">{{ trans('app.release_did_number') }}</span>
