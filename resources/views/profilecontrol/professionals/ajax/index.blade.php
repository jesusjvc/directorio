            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    ...
                    <div class="pull-right">
                        <span class="btn btn-xs btn-success" data-toggle="modal" data-target="#ajaxmodel"
                              data-remote="{{ url(Session::get('guard') . '/professionals/create') }}">
                ...
                <div class="panel-body">
                    @if(Session::get('guard') == 'profilecontrol')
                        @php
                            $totalProfessionals = $profile->users()->where('isprofessional', 1)->count();
                        @endphp
                        @if (($profile->subscription_package != null) && ($profile->subscription_package->limit_professional_accounts != 0) && ($totalProfessionals >= $profile->subscription_package->limit_professional_accounts))
                            <p class="text-danger text-center">
                                {{ trans('app.your_type_limit_has_been_reached_please_upgrade_your_subscription_package', ["type" => strtolower(trans('app.professional_provider')), "limit" => $totalProfessionals]) }}
                            </p>
                        @endif
                    @endif
                    @if(count($users) > 0)
                            ...
                            <table class="table">
                                ...
                                @foreach($users as $user)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ url(Session::get('guard') . '/professionals/' . $user->id . '/send_mail') }}"
                                               class="fetchajaxpage"
                                        ...
                                        <td>
                                            @if(is_numeric($user->mobile_no))
                                                @if (Session::get('app_settings')->disable_sms == 0)
                                                    <a href="{{ url(Session::get('guard') . '/professionals/' . $user->id . '/send_sms') }}"
                                            ...
                                            <div class="oneline m-t-5">
                                                @if($user->agenda->branches->count() == 0)
                                                    <span class="btn btn-primary btn-xs" data-toggle="modal"
                                                          ...
                                                          data-remote="{{ url(Session::get('guard') . '/professionals/' . $user->id . '/linkbranch') }}">{{ trans('app.link') }}
                                        </span>
                                                ...
                                                @else
                                                    <span class="btn btn-primary btn-xs" data-toggle="modal"
                                                          data-target="#ajaxmodel"
                                                          data-remote="{{ url(Session::get('guard') . '/professionals/' . $user->id . '/linkbranch') }}">{{ trans('app.link_more') }}
                                        </span>
                                                @endif
                                            </div>
                                            @if($user->agenda->branches->count() > 0)
                                                ...
                                                <span class="btn btn-default btn-xs">
                                                    {{ $branch->branch_name }}
                                                        ...
                                                        <span class="btn btn-danger btn-xs postconfirm"
                                                              ...
                                                              data-reloadurl="{{ url(Session::get('guard') . '/professionals') }}"
                                                              data-posturl="{{ url(Session::get('guard') . '/professionals/' . $user->id . '/revoke_branch/' . $branch->id) }}">{{ trans('app.revoke') }}
                                                </span>
                                                        @endforeach
                                                        @endif
                                                    </div>
                                        ...
                                        <span class="btn btn-primary btn-xs" data-toggle="modal"
                                              data-target="#ajaxmodel"
                                              data-remote="{{ url(Session::get('guard') . '/professionals/'.$user->id.'/edit') }}">{{ trans('app.edit') }}</span>
                                        </td>
                                        <td>
                                            <form role="form" method="POST"
                                                  action="{{ url(Session::get('guard') . '/professionals/'.$user->id) }}"
                                                  id="{{ $user->id }}">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
                                                <span class="btn btn-danger btn-xs postconfirm"
                                                      ...
                                                      data-reloaddiv="{{ $sectiontoreload }}"
                                                      data-reloadurl="{{ url(Session::get('guard') . '/professionals') }}"
                                                      data-posturl="{{ url(Session::get('guard') . '/professionals/'.$user->id) }}">{{ trans('app.delete') }}
                                                </span>
                                            </form>
                                        </td>
                                    </tr>
