<table class="table table-hover">
    ...
    @if($quotation_items->count() > 0)
            ...
            <tr>
                <td>
                    <a class="postconfirm hand tableicon"
                       ...
                       data-posturl="{{ url(Session::get('guard') . '/quotations/' . $quotation->id . '/items/' . $item->id . '/remove') }}">
        ...
        <tr>
            ...
            <th class="text-right">{{ strtoupper(trans('app.total_in_currency',["currency" => $quotation_currency->code])) }} @if($profile->profile_billing->tax_enabled == 1) <i><small>{{ trans('app.excl_tax') }}</small></i> @endif</th>
            @if(strlen($quotation_currency->symbol) > 2)
                <th class="text-right">{{ $quotation_currency->code }} {{ number_format($doc_total,2) }}</th>
            @else
                <th class="text-right">{{ $quotation_currency->symbol }}{{ number_format($doc_total,2) }}</th>
            @endif
        </tr>
        @if(substr(CustomHelper::getDecimals(str_replace(',','',$doc_total)),2))
            <tr>
                ...
                @if(strlen($quotation_currency->symbol) > 2)
                    <th class="text-right">{{ number_format(str_replace(',','',$doc_total),2) }} {{ $quotation_currency->code }}</th>
                @else
                    <th class="text-right">{{ $quotation_currency->symbol }}{{ number_format(str_replace(',','',$doc_total),2) }}</th>
                @endif
            </tr>
        @endif
    @endif
