<form role="form" method="POST" action="{{ url(Session::get('guard') . '/subscriptions/' . $subscription->id . '/add_item') }}" id="idForm"
      reloadiv="reload" reloadurl="{{ url(Session::get('guard') . '/subscriptions/' . $subscription->id . '/items') }}">
    {{ csrf_field() }}
    <div class="">
        ...
        <div class="col-md-2">
            ...
            <div class="input-group">
                <input type="text" name="none" id="item_amount" class="form-control" autocomplete="off" readonly>
                <div class="input-group-addon">{{ $profile->profile_billing->default_currency }}</div>
            </div>
        </div>

        <div class="col-md-2">
            <label class="oneline">{{ trans('app.custom_amount') }}</label>
            <div class="input-group">
                <input type="text" name="item_amount" id="custom_amount" autocomplete="off" class="form-control">
                <div class="input-group-addon">{{ $profile->profile_billing->default_currency }}</div>
            </div>
        </div>
        ...
        <div class="col-md-1 text-center">
            ...
            <a class="fetchajaxpage" href="{{ url(Session::get('guard') . '/subscriptions/' . $subscription->id . '/items') }}">
